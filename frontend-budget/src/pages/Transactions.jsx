...
import { useTranslation } from 'react-i18next'; // 💡 IMPORT DES TRADUCTIONS
import { 
  LayoutDashboard, Tag, LogOut, Wallet, PlusCircle, Trash2, AlertCircle, CheckCircle2, ArrowRightLeft, ChevronDown, Search, Mic,
...
function Transactions() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ titre: '', montant: '', type: 'depense', categorieId: '', description: '', date: new Date().toISOString().split('T')[0] });
      fetchData();
  ...
  // 💡 Format de la date dynamique selon la langue
        ...
        <div className="flex-1 px-8 pb-8 max-w-[1600px] w-full mx-auto overflow-hidden flex flex-col min-h-0">
          ...
          <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-8 min-h-0">
            ...
            <div className="lg:col-span-5 flex flex-col min-h-0">
              <div className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-700/50 overflow-y-auto custom-scrollbar">
                ...
                {status.message && (
                  <div className={`p-3 rounded-xl mb-4 text-sm font-medium ${status.type === 'success' ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400' : 'bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400'}`}>
                    {status.message}
                  </div>
                )}
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <div className="flex-1 bg-slate-50 dark:bg-slate-700/50 p-1 rounded-xl flex gap-1 border border-slate-200 dark:border-slate-600">
                        <button type="button" onClick={() => setQuickDate(0)} className={`flex-1 py-1.5 rounded-lg text-sm font-bold transition ${formData.date === new Date().toISOString().split('T')[0] ? 'bg-blue-500 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-600'}`}>{t('transactions.todayBtn', 'Auj.')}</button>
                      ...
                      <input type="date" required className="px-3 py-1.5 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl outline-none text-sm cursor-pointer" value={formData.date} onChange={(e) => setFormData({...formData, date: e.target.value})} />
                    </div>
                  </div>
                  ...
                  <div className="relative">
                    ...
                    {isCategoryOpen && (
                      <>
                        ...
                        <div className="absolute z-20 w-full mt-1 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-xl rounded-xl max-h-48 overflow-y-auto custom-scrollbar">
                          {categories.map(cat => (
                            <div key={cat._id} onClick={() => { setFormData({ ...formData, categorieId: cat._id }); setIsCategoryOpen(false); }} className="flex items-center gap-3 px-4 py-2 hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer transition">
                              <div className="w-8 h-8 rounded-full flex items-center justify-center text-white" style={{ backgroundColor: cat.couleur || '#cbd5e1' }}><RenderIcon name={cat.icone} size={16} /></div>
                  ...
                  <div>
                    ...
                    <textarea placeholder={t('transactions.descPlaceholder', 'Ajoutez des détails')} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 transition resize-none h-16 text-slate-800 dark:text-white text-sm" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} />
                  </div>

                  <button type="submit" className="w-full bg-blue-600 text-white font-bold py-3 rounded-xl hover:bg-blue-700 transition shadow-lg text-sm">
            ...
            <div className="lg:col-span-7 flex flex-col min-h-0">
              <div className="bg-white dark:bg-slate-800 p-8 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-700/50 flex-1 flex flex-col min-h-0">
                ...
                <div className="flex-1 overflow-y-auto pr-4 custom-scrollbar space-y-4">
                  {filteredTransactions.map((tx) => (
                    <div key={tx._id} className="flex flex-col p-5 border border-slate-100 dark:border-slate-700/50 hover:border-blue-200 rounded-2xl transition group bg-white dark:bg-slate-800">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-full flex items-center justify-center text-white shadow-sm" style={{ backgroundColor: tx.categorie?.couleur || '#cbd5e1' }}>
                            <RenderIcon name={tx.categorie?.icone || 'FileText'} size={20} />
                          </div>
                          <div>
                            <h4 className="font-bold text-lg leading-tight">{tx.titre}</h4>
                            <p className="text-xs text-slate-500 mt-1">{tx.categorie?.nom || t('transactions.general', 'Général')} • {formaterDate(tx.date)}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-6">
                          <span className={`text-lg font-bold ${tx.type === 'revenu' ? 'text-emerald-500' : 'text-rose-600'}`}>{tx.type === 'revenu' ? '+' : '-'}{formatDevise(tx.montant)}</span>
                          <button onClick={() => handleDelete(tx._id)} className="text-slate-300 hover:text-rose-500 transition opacity-0 group-hover:opacity-100 p-2"><Trash2 size={18} /></button>
                        </div>
                      </div>
                      {tx.description && (
                        <div className="mt-4 ml-16 bg-slate-50 dark:bg-slate-700/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700">
                          <p className="text-sm text-slate-600 dark:text-slate-300 italic">"{tx.description}"</p>
                        </div>
                      )}
                    </div>
                  ))}
